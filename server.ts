...
async function startServer() {
  ...
  // Fetch custom CartFlows swatches from WP REST API (Cached)
  ...
  const fetchSwatches = async () => {
      ...
      try {
          ...
          const res = await axios.get(`${wpUrl}/wp-json/chils/v1/swatches`, { timeout: 4000 });
          if (res.data && typeof res.data === 'object' && !res.data.code) {
              const swatches: Record<string, any> = {};
              Object.values(res.data).forEach((term: any) => {
  ...
  const mapProduct = (wcProduct: any, resolvedMediaUrls: Record<string, string> = {}, swatchesData: Record<string, any> = {}) => {
        ...
        if (v.meta_data && Array.isArray(v.meta_data)) {
            ...
            metaKeys.forEach(key => {
                ...
                if (meta && typeof meta.value === 'string') {
                    ...
                    // Sometimes serialized arrays parse as objects
            ...
            if (v.meta_data && Array.isArray(v.meta_data)) {
                ...
                metaKeys.forEach(key => {
                    ...
                    if (meta && typeof meta.value === 'string') {
                        ...
                        Object.values(meta.value).forEach((id: any) => allGalleryIds.add(String(id)));
                    }
                });
            }
        });
  ...
  app.post("/api/checkout/phonepe/retry", async (req, res) => {
    try {
      ...
      // Verify signal matches orderId to prevent unauthorized requests
      ...
      // 2. Verify order status allows retry (pending, failed, cancelled, on-hold)
      ...
      // 3. Generate a new unique merchantTransactionId
      const merchantTransactionId = `${orderId}_${Date.now()}`;

      // 4. Update WooCommerce order status to pending and set the new merchantTransactionId
      ...
      // 5. Build PhonePe standard checkout request
      const phonePeClient = getPhonePeClient();
      if (!phonePeClient) {
        return res.status(400).json({ 
          success: false,
      ...
      const requestBuilder = StandardCheckoutPayRequest.builder()
        ...
        .amount(amountPaise)
        .redirectUrl(redirectUrl)
        .message(`Payment retry for Order #${orderId}`);
...
`;
  ...
  app.post("/api/amazon/export", authenticateToken, async (req: any, res) => {
    try {
      const adminEmails = ['chilsandco@gmail.com', 'chilsandco.com@gmail.com'];
      const isAdmin = adminEmails.some(email => email.toLowerCase() === req.user.email.toLowerCase());
      if (!isAdmin) {
      ...
      let currentRow = 6; // Row 7 in Excel is index 6
      ...
      for (const product of products) {
        ...
        // 1. Write Parent Row
        const parentCols: { [key: number]: any } = {
          0: parentSku, // Col 1 SKU
          1: "SHIRT", // Col 2 Product Type
          2: "full_update", // Col 3 Listing Action
          3: "parent", // Col 4 Parentage Level
          5: "SIZE/COLOR", // Col 6 Variation Theme Name
          6: product.name, // Col 7 Item Name
          8: brand, // Col 9 Brand Name
          19: enrich.modelName, // Col 20 Model Name
          20: manufacturer, // Col 21 Manufacturer
          31: product.description, // Col 32 Product Description
          32: enrich.bulletPoints?.[0] || "", // Col 33 Bullet Point 1
          33: enrich.bulletPoints?.[1] || "", // Col 34 Bullet Point 2
          34: enrich.bulletPoints?.[2] || "", // Col 35 Bullet Point 3
          35: enrich.bulletPoints?.[3] || "", // Col 36 Bullet Point 4
          36: enrich.bulletPoints?.[4] || "", // Col 37 Bullet Point 5
          37: enrich.genericKeywords, // Col 38 Generic Keywords
          45: "unisex-adult", // Col 46 Department Name
          46: "unisex", // Col 47 Target Gender
          47: "adult", // Col 48 Age Range Description
          53: product.material || "100% Cotton", // Col 54 Material
          56: enrich.apparelFabricWeightClass === "Heavyweight" ? "French Terry" : "Cotton Knit", // Col 57 Fabric Type
          65: parentSku, // Col 66 Part Number
          69: product.care || "Machine Wash", // Col 70 Care Instructions
          104: importer, // Col 105 Importer Contact
          109: packer, // Col 110 Packer Contact
          118: enrich.sleeveLengthDescription || "Long Sleeve", // Col 119 Sleeve Length Description
          120: enrich.closureType || "Pull-On", // Col 121 Closure Type
          130: enrich.apparelFabricWeightClass || "Heavyweight", // Col 131 Weight Class
          131: "India", // Col 132 Garment Size Country
          132: 70, // Col 133 Shoulder to bottom hem length
          133: "Centimeters", // Col 134 Shoulder to bottom hem length unit
          134: enrich.apparelFabricStretch || "Low Stretch", // Col 135 Stretch
          135: enrich.fitToSizeSentiment || "True to Size", // Col 136 Fit Sentiment
          139: enrich.itemWeightGrams || 450, // Col 140 Item Weight
          140: "Grams", // Col 141 Item Weight Unit
          142: "New", // Col 143 Item Condition
          190: parseInt(pkgLength, 10), // Col 191 Package Length
          191: "Centimeters", // Col 192 Package Length Unit
          192: parseInt(pkgWidth, 10), // Col 193 Package Width
          193: "Centimeters", // Col 194 Package Width Unit
          194: parseInt(pkgHeight, 10), // Col 195 Package Height
          195: "Centimeters", // Col 196 Package Height Unit
          196: parseInt(pkgWeight, 10), // Col 197 Package Weight
          197: "Grams", // Col 198 Package Weight Unit
          198: origin // Col 199 Country of Origin
        };
        ...
        for (const v of variations) {
          ...
          const childCols: { [key: number]: any } = {
            0: childSku, // Col 1 SKU
            1: "SHIRT", // Col 2 Product Type
            2: "full_update", // Col 3 Listing Action
            3: "child", // Col 4 Parentage Level
            4: parentSku, // Col 5 Parent SKU
            5: "SIZE/COLOR", // Col 6 Theme Name
            6: `${product.name} (Color: ${color}, Size: ${size})`, // Col 7 Item Name
            8: brand, // Col 9 Brand Name
            19: enrich.modelName, // Col 20 Model Name
            20: manufacturer, // Col 21 Manufacturer
            31: product.description, // Col 32 Product Description
            ...
            46: "unisex", // Col 47 Target Gender
            47: "adult", // Col 48 Age Range Description
            48: "ASIAN", // Col 49 Shirt Size System
            49: "ALPHA", // Col 50 Shirt Size Class
            50: size, // Col 51 Shirt Size Value
            52: "Regular", // Col 53 Shirt Body Type
            53: product.material || "100% Cotton", // Col 54 Material
            56: enrich.apparelFabricWeightClass === "Heavyweight" ? "French Terry" : "Cotton Knit", // Col 57 Fabric Type
            62: mappedColor, // Col 63 Color Map
            63: color, // Col 64 Color
            65: childSku, // Col 66 Part Number
            69: product.care || "Machine Wash", // Col 70 Care Instructions
            104: importer, // Col 105 Importer Contact
            109: packer, // Col 110 Packer Contact
            118: enrich.sleeveLengthDescription || "Long Sleeve", // Col 119 Sleeve Length Description
            ...
            140: "Grams", // Col 141 Item Weight Unit
            142: "New", // Col 143 Item Condition
            169: v.stockQuantity || 0, // Col 170 Quantity (IN)
            170: parseInt(handling, 10), // Col 171 Handling Time (IN)
            173: parseFloat(v.price || product.price), // Col 174 Your Price INR
            190: parseInt(pkgLength, 10), // Col 191 Package Length
            191: "Centimeters", // Col 192 Package Length Unit
            192: parseInt(pkgWidth, 10), // Col 193 Package Width
            193: "Centimeters", // Col 194 Package Width Unit
      ...
      res.setHeader("Content-Disposition", `attachment; filename=Amazon_Bulk_Listing_Feed_${new Date().toISOString().split('T')[0]}.xlsm`);
      res.send(buffer);
  ...
  app.post("/api/admin/whatsapp/broadcast", authenticateToken, async (req: any, res) => {
    try {
      const adminEmails = ['chilsandco@gmail.com', 'chilsandco.com@gmail.com'];
      const isAdmin = adminEmails.some(email => email.toLowerCase() === req.user.email.toLowerCase());
      if (!isAdmin) {
      ...
      if (phoneNumbers.length === 1 && phoneNumbers[0] === "all_customers") {
        ...
        if (wc) {
          ...
          targets = customersRes.data.map((c: any) => c.billing?.phone).filter(Boolean);
        } else {
          targets = [];
        }
      }
    ...
    try {
      ...
      if (topic === 'order.updated' || topic === 'order.refunded') {
        ...
        if (status === 'refunded') {
          ...
          // Send build dispatched/completed email
  ...
  <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #050505; border: 1px solid #1a1a1a; padding: 40px;">
    ...
    <tr>
      <td align="center" style="padding-top: 40px; border-top: 1px solid #111; color: #444; font-family: monospace; font-size: 8px; letter-spacing: 0.2em; text-transform: uppercase; line-height: 1.6;">
        CHILS & CO. &copy; ${new Date().getFullYear()}<br/>
...
${xmlItems}
  ...
  if (!IS_PROD) {
    ...
    if (fs.existsSync(distPath)) {
      ...
      // ── Social Media Crawler OG Tag Injection ──────────────────────────
      // WhatsApp, Facebook, Instagram, Twitter, LinkedIn, etc. send crawler
      // bots that don't execute JavaScript. Since this is a React SPA, the
      // crawlers only see the static index.html meta tags. This middleware
      // intercepts crawler requests for product pages and injects product-
      // specific OG tags so link previews show the correct product image,
      ...
      app.get("*", async (req, res) => {
        ...
        // For non-crawler requests, just serve the SPA index.html as-is
        if (!isCrawler) {
          return res.sendFile(indexPath);
        }

        // For crawler requests, check if this is a product page
        const productMatch = req.path.match(/^\/product\/(\d+)/);
        
        if (!productMatch) {
          // Non-product page crawler request — serve default OG tags (already in HTML)
          return res.sendFile(indexPath);
        }

        // Product page crawler request — inject product-specific OG tags
        ...
        try {
          ...
          const productDesc = (product.short_description || product.description || "")
            .replace(/<[^>]*>/g, "")  // Strip HTML tags
            .replace(/&amp;/g, "&").replace(/&#039;/g, "'").replace(/&quot;/g, '"')
            .replace(/&nbsp;/g, " ")
            .substring(0, 200)
            .trim() || "Premium luxury garment by Chils & Co.";
          
          // Get the first product image (use direct URL, not the /api/media proxy)
          ...
          // Read the index.html and replace OG meta tags
          ...
          // Replace OG meta tag content values using regex
          html = html.replace(
          ...
          // Replace Twitter card meta tags too
          html = html.replace(
          ...
          // Add product-specific meta tags (price) before closing </head>
          const productMeta = `
    ...
    <meta property="product:price:currency" content="INR" />`;
          ...
          // Fallback: serve the default index.html
...
// ==========================================
// CHILS & CO. - BREVO EMAIL NOTIFICATION SYSTEM
// ==========================================

const BREVO_API_KEY = process.env.BREVO_API_KEY || "";
const SENDER_EMAIL = "hello@chilsandco.com";
const LOGO_URL = "https://res.cloudinary.com/ddatd5ruz/image/upload/v1774668881/chils_simple_logo_transparent_kdfrfk.png";
...
// 1. Welcome Email (Identity Initialized)
function compileWelcomeHtml(email: string, firstName: string) {
  return `
<!DOCTYPE html>
<html>
<body style="background-color: #000000; color: #ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 0; padding: 40px 20px;">
...
// 2. Order Confirmation (Transmission Confirmed)
function compileOrderConfirmationHtml(order: any) {
  const signalId = toSignalId(order.id);
  const itemsList = order.line_items.map((item: any) => {
    const size = item.meta_data?.find((m: any) => m.key === "pa_size" || m.key === "size")?.value || "";
    return `
  ...
  const address = order.shipping
    ? `${order.shipping.first_name || ''} ${order.shipping.last_name || ''}<br/>${order.shipping.address_1 || ''}<br/>${order.shipping.city || ''}, ${order.shipping.state || ''} ${order.shipping.postcode || ''}<br/>IN`
    : 'No address provided';

  const buyer = order.billing
    ? `${order.billing.first_name || ''} ${order.billing.last_name || ''}<br/>${order.billing.phone || ''}`
    : 'Guest';
...
function compileAdminNotificationHtml(order: any) {
  const signalId = toSignalId(order.id);
  const itemsList = order.line_items.map((item: any) => `
...
// 5. Refund Confirmed (Recoupment Complete)
function compileRefundHtml(order: any) {
  const signalId = toSignalId(order.id);
  return `
<!DOCTYPE html>
<html>
...
// Trigger handlers
async function triggerWelcomeEmail(email: string, firstName: string) {
...
async function triggerOrderConfirmationEmails(order: any) {
  if (!order) return;
...
async function triggerReturnRequestEmails(orderId: string, reason: string, description: string, products: any) {
...
async function triggerRefundEmail(order: any) {
  if (!order) return;
  const customerEmail = order.billing?.email;
  const customerName = `${order.billing?.first_name || ''} ${order.billing?.last_name || ''}`.trim() || "Customer";
  const signalId = toSignalId(order.id);
...
async function sendWhatsAppMessage(toPhone: string, templateName: string, languageCode: string = "en_US", components: any[] = []) {
  ...
  try {
    ...
    if (components.length > 0) {
      (data.template as any).components = components;
    }
...
async function triggerWhatsAppOrderConfirmation(order: any) {
  if (!order) return;
  ...
  const itemsSummary = order.line_items?.map((i: any) => `${i.quantity}x ${i.name}`).join(", ") || "Your items";

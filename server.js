...
`;
  ...
  app.post("/api/amazon/export", authenticateToken, async (req, res) => {
    try {
      ...
      const jsonTempPath = path.join(process.cwd(), `temp_${tempId}.json`);
      ...
      await fs.promises.writeFile(jsonTempPath, JSON.stringify(payload, null, 2), "utf-8");
      const pythonScript = path.join(process.cwd(), "scripts", "generate_feed.py");
      const { exec } = await import("child_process");
      const handleExcelResult = async (err, stdout, stderr) => {
        try {
          if (err) {
            console.error("[AMAZON EXPORT] Python error:", stderr || err.message);
            await fs.promises.unlink(jsonTempPath).catch(() => {
            });
            return res.status(500).json({ message: "Failed to generate Excel spreadsheet via python.", error: stderr || err.message });
          }
          console.log("[AMAZON EXPORT] Excel generation stdout:", stdout);
          if (!fs.existsSync(xlsmTempPath)) {
            await fs.promises.unlink(jsonTempPath).catch(() => {
            });
            return res.status(500).json({ message: "Excel output file was not generated by script." });
          }
          ...
          console.error("[AMAZON EXPORT] Inner error:", innerErr);
          await fs.promises.unlink(jsonTempPath).catch(() => {
          });
          await fs.promises.unlink(xlsmTempPath).catch(() => {
          });
      ...
      res.status(500).json({ message: error.message || "Failed to initiate spreadsheet export." });
    }
  });
